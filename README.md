...
이번 글에서 앱을 만들면서 MVC패턴을 살펴보고. 앱을 만들면서 필요했던  라이브러리들인 Alamofire에 대한 정보 나아가서 프로젝트를 발전시키기 위해서 했던 노력들을 공유해보고자 한다. 프로젝트에 대해서 궁금한 점이 있거나 부족한 점이 보인다면 이슈 탭에 코멘트 언제든지 환영. 
...
유저가 가져야할 데이터는 뭐가 있을까? 사용자의 현재 위치 끝.
...
지도 객체는 지도 중심을 기준으로 가맹점 정보를 호출하기 위해 지도의 가운데 위도 경도와,  가맹점들에 대한 객체를 가지고 있어야한다. 
...
데이터를 호출하는 시점은 앱의 시작이 될 수 도 있고 앱을 사용하면서 특정 상황에 불러올 수도 있다. 이 호출은 모델에서 직접 하는 것이 아니기 때문에 일단 호출하는 
...
__2.1.__ 비동기 처리: 비동기 프로세스 동기로 처리하기
